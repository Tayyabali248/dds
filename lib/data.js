...
// Static values that are the same for every entry (matches what was
// established filling the real PTCL DDS New Customer form by hand).
const EXCHANGE = 'Rahim Yar Khan';
const REGION = 'MTR';
const COMPETITION = 'Local';

// Fully self-contained entry: every field the DDS form needs, so callers
// (the extension's content script) don't need to duplicate these constants.
function buildEntry() {
...
function buildEntries(count) {
  return Array.from({ length: count }, () => buildEntry());
}
